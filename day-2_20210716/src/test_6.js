...
describe("Verify that function numberSequence return correct data ", function () {
  ...
  it("Function numberSequence with n=7 m=149 return string '13,14,15,16,17,18,19'", function () {
    ...
    it("Function numberSequence with n=1 m=423 return string '21'", function () {
      assert.isNotNumber( numberSequence(1, 423), "return correct type of data - '21' is a string");
    });

    it("Function numberSequence with n=7 m=33 return string '6,7,8,9,10,11,12' with length 16", function () {
      assert.lengthOf( numberSequence(7, 149), 16, "string '6,7,8,9,10,11,12' has length 16")
    });
...
describe("Verify that function numberSequence accepts incorrect parameters", function () {
  ...
  it("Function numberSequence with n = 0 m = 23 return an empty string ", function () {
      assert.equal( numberSequence(0, 23), `{status:'failed', reason:TypeError: function should accepts parameter n > 0, n should be > 0}`
      );
    });

  it("Function numberSequence with n = -3 m = 217 return Error ", function () {
      assert.equal( numberSequence(-3, 217), `{status:'failed', reason:TypeError: function should accepts positive number, n should be a positive number}`
      );
    });

  it("Function numberSequence with n = 5.9 m = 331 return Error ", function () {
      assert.equal( numberSequence(5.9, 331), `{status:'failed', reason:TypeError: function should accepts an integer number, n should be an integer number}`
      );
    });

  it("Function numberSequence with n = 3 m = [25] return Error ", function () {
      assert.equal( numberSequence(3, [25]), `{status:'failed', reason:TypeError: function should accepts only numbers, m should be a number}`
      );
    });
